...
load /home/j1c/plotResults/zerofriction_paper/momenum_flux_cospectra/Feb28a_sigma=4.mat

C = combCp2d;
%C(abs(C)<max(C(:))*0.001) = NaN;
C_p = C; C_p(C_p<0) = 0;
C_n = C; C_n(C_n>0) = 0;
contour(lat(latRange), combPhaseSpeed, C_p, 6, 'linecolor', 'k');
hold on
contour(lat(latRange), combPhaseSpeed, C_n, 6, 'linecolor', 'k','LineStyle',':');
ylim([-35 40])
xlim([0, 90])
plot(lat,umean./cos(lat*pi/180),'--k')

set(gca,'xtick',xTick,'xticklabel',xTickLabelBlank)

     
axes(ha(2));
load /home/j1c/plotResults/zerofriction_paper/momenum_flux_cospectra/Feb28e_sigma=4.mat
...
axes(ha(3));
load /home/j1c/plotResults/zerofriction_paper/momenum_flux_cospectra/Feb28e_sigma=26.mat

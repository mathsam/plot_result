...
ax.semilogx(k, -np.cumsum(eddy_eddy)/eps, linewidth=linewidth1, label='Eddy-eddy')
ax.semilogx(k, -np.cumsum(total-eddy_eddy)/eps, linewidth=linewidth1, label='Eddy-mean')
ax.semilogx(k, -np.cumsum(total)/eps, linewidth=linewidth1, label='Total')
ax.legend(loc='best',frameon=False,fontsize=fontsize3)

ax.plot([k_epsilon, k_epsilon], 
        [-0.9, -1.1],
        '--k', linewidth=linewidth2)
ax.text(k_epsilon, -1.1, r'$k_\varepsilon$')
ax.plot([kf, kf], 
        [0, 0.25],
        '--k', linewidth=linewidth2)
ax.text(kf, 0.25, r'$k_f$')
ax.set_xlim(1, 511)
ax.set_ylim(-1.2, 0.4)
ax.xaxis.set_ticklabels([])
ax.set_ylabel(r'$\mathcal{F}(k)/\varepsilon$')
